...
// 각 관심사에 대한 추천 영양제 데이터
const recommendedSupplementsData = {
  cognitive: {
    title: '인지 기능 지원 영양제',
    supplements: [
      { name: '오메가-3', description: '뇌세포막의 주요 구성 성분으로, 기억력과 학습 능력에 도움을 줄 수 있습니다.' },
      { name: '은행잎 추출물 (징코빌로바)', description: '뇌 혈액 순환을 개선하여 인지 기능 저하를 늦추는 데 도움을 줄 수 있습니다.' },
    ],
  },
  sleep: {
    title: '수면의 질 개선 영양제',
    supplements: [
      { name: '락티움', description: '스트레스로 인한 긴장을 완화하여 수면의 질 개선에 도움을 줄 수 있습니다.' },
      { name: '마그네슘', description: '신경계를 안정시키고 근육을 이완시켜 편안한 수면을 돕습니다.' },
    ],
  },
  fatigue: {
    title: '피로 회복 및 활력 증진 영양제',
    supplements: [
      { name: '홍경천 추출물', description: '스트레스로 인한 피로 개선에 도움을 줄 수 있습니다.' },
      { name: '코엔자임 Q10', description: '세포 에너지 생성을 촉진하여 신체 활력을 높이는 데 기여합니다.' },
    ],
  },
  eye: {
    title: '눈 건강 지원 영양제',
    supplements: [
        { name: '루테인 & 지아잔틴', description: '황반 색소 밀도를 유지하여 눈의 노화와 피로를 줄여줍니다.' },
        { name: '비타민 A', description: '어두운 곳에서의 시각 적응을 위해 필요하며, 눈의 점막을 건강하게 유지합니다.' },
    ],
  },
  liver: {
    title: '간 건강 지원 영양제',
    supplements: [
        { name: '밀크씨슬 (실리마린)', description: '간세포를 보호하고 간 기능 개선에 도움을 줄 수 있는 대표적인 성분입니다.' },
    ],
  },
  default: {
    title: '추천 영양제',
    supplements: [
        { name: '선택된 관심사에 맞는 영양제를 추천합니다.', description: '' },
    ],
  }
};
    ...
    <div className="flex flex-col h-screen bg-white">
      ...
      <div className="flex-grow overflow-y-auto p-6">
        ...
        <section className="mt-10">
          <h2 className="text-xl font-bold text-[#0028A0]">{currentSupplementData.title}</h2>
          <div className="mt-4 space-y-4">
            {currentSupplementData.supplements.map((item, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-bold text-md text-blue-800">{item.name}</h3>
